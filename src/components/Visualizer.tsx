...
export default function Visualizer({ state, isWakeWordListening }: VisualizerProps) {
  const getRingAnimation = (index: number, reverse: boolean = false) => {
    const baseSpeed = state === "listening" ? 3 : state === "processing" ? 1.5 : state === "speaking" ? 2 : (isWakeWordListening ? 8 : 15);
    return {
  ...
  const getPulseAnimation = () => {
    ...
    // Idle / Wake word phase
    return {
      scale: isWakeWordListening ? [1, 1.02, 1] : [1, 1.01, 1],
      opacity: isWakeWordListening ? [0.5, 0.8, 0.5] : [0.4, 0.6, 0.4],
      transition: { duration: isWakeWordListening ? 2 : 4, repeat: Infinity, ease: "easeInOut" }
    };
  };
    ...
    switch (state) {
      ...
      default: 
        if (isWakeWordListening) {
          return { color: "rgba(139, 92, 246, 0.8)", glow: "shadow-violet-500/30", border: "border-violet-500/40" };
        }
        return { color: "rgba(6, 182, 212, 0.8)", glow: "shadow-cyan-500/40", border: "border-cyan-500/50" }; // Cyan for idle
    }
  };
